...
| **Phase 25**| Windows NSIS Setup Installer (.exe) Optimization, Security Hardening & Runtime Engine Fix | ✅ Completed | Configured `electron-builder` with standard Windows NSIS setup wizard (`.exe` installer); Added automatic Desktop shortcut and Start Menu creation; Generated multi-resolution `electron/icon.ico`; Configured writeable `userData` database directory in `electron/main.js` guaranteeing read/write permissions for standard Windows users; Fixed dot-folder exclusion of `node_modules/.prisma` query engine by explicitly mapping `extraResources` and adding self-healing fallback copy in `main.js` (resolving packaged 500 internal server error); Added dynamic port allocation to eliminate localhost:3000 collision crashes; Applied maximum LZMA compression and development file exclusions (327 MB → 189 MB total self-contained offline installer); Completely eliminated all hardcoded demo passwords from `/api/auth/login` (pure SQLite bcrypt authentication); Documented 2-click client download & install runbook in `docs/OFFLINE_DESKTOP_GUIDE.md`. |
