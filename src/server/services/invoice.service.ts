...
export async function getInvoiceById(id: string): Promise<InvoiceDetailRecord | null> {
  try {
    ...
    return {
      ...
      items,
      companyName: company?.name || "PharmaDist Wholesale Distributors Ltd.",
      companyAddress: company?.address || "Plot 14, Commercial Zone, Korangi Industrial Area, Karachi",
      companyPhone: company?.phone || "+92 21 35000111",
      companyEmail: company?.email || "info@pharmadist.pk",
      companyDrugLicense: company?.drugLicenseNo || "DRAP-KHI-09182-W",
      companyTradeLicense: company?.tradeLicenseNo || "TRAD-KHI-2024-8849",
      companyTin: company?.taxIdTin || "NTN-8291039-1",
