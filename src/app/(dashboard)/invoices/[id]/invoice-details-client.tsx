      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 print:hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            Print {activeTab === "INVOICE" ? "Invoice" : "Challan"}
          </Button>
        </div>
      </div>

      {/* 2. Printable Invoice Document Container */}
      <div className="bg-card border border-border/80 rounded-2xl p-8 sm:p-12 shadow-sm print:border-none print:shadow-none print:p-0 print:m-0 text-foreground font-sans">
        {/* Header Block */}
        <div className="flex flex-col sm:flex-row justify-between items-start pb-6 border-b-2 border-primary/20 gap-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground max-w-sm">{invoice.companyAddress}</p>
            <div className="text-[11px] text-muted-foreground space-y-0.5 pt-1">
              <div>Phone: <span className="font-mono text-foreground">{invoice.companyPhone}</span> • Email: {invoice.companyEmail}</div>
              <div>DGDA Drug License: <strong className="font-mono text-foreground">{invoice.companyDrugLicense}</strong> • Trade Lic: {invoice.companyTradeLicense}</div>
            </div>
          </div>
          ...
          <div className="sm:text-right space-y-1">
            <Badge
              variant="outline"
              className="text-sm px-3 py-1 font-bold tracking-wider uppercase border-primary/40 bg-primary/5 text-[#0071E3]"
            >
              {activeTab === "INVOICE" ? "Wholesale Tax Invoice" : "Goods Delivery Challan"}
            </Badge>
            <div className="font-mono font-extrabold text-xl text-foreground pt-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 py-6 border-b border-border/60 text-xs">
          <div className="space-y-1">
            <span className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider">
              Billed To (Licensed Pharmacy / Client)
            </span>
            <div className="text-sm font-bold text-foreground">{invoice.customerName}</div>
            ...
            <div className="pt-1">
              <span className="inline-block px-2 py-0.5 rounded bg-muted font-mono font-bold text-foreground">
                Customer Drug License: {invoice.customerDrugLicense}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-1 sm:text-right">
            ...
            <div className="text-sm font-bold text-foreground">{invoice.salesmanName}</div>
        ...
        <div className="py-6 overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b-2 border-border/80 font-bold uppercase tracking-wider text-muted-foreground">
                <th className="py-2.5 px-2">#</th>
                <th className="py-2.5 px-2">Medicine Brand & Generic</th>
                <th className="py-2.5 px-2">Batch #</th>
                <th className="py-2.5 px-2">Expiry</th>
                <th className="py-2.5 px-2 text-center">Billed Qty</th>
                <th className="py-2.5 px-2 text-center">Bonus</th>
                {activeTab === "INVOICE" && (
                  <>
                    <th className="py-2.5 px-2 text-right">Unit TP</th>
                    <th className="py-2.5 px-2 text-right">Unit MRP</th>
            ...
            <tbody className="divide-y divide-border/40 font-mono">
              {invoice.items.map((item, idx) => (
                <tr key={idx} className="hover:bg-muted/10">
                  ...
                  <td className="py-2.5 px-2 font-sans font-semibold text-foreground">
                    <div>{item.medicineName}</div>
                    <div className="text-[10px] text-muted-foreground font-normal">
                      {item.genericName} • {item.dosageForm}
                    </div>
                  </td>
                  <td className="py-2.5 px-2 font-bold">{item.batchNumber}</td>
                  <td className="py-2.5 px-2 text-muted-foreground">{formatDate(item.expiryDate)}</td>
                  ...
                  {activeTab === "INVOICE" && (
                    <>
                      <td className="py-2.5 px-2 text-right">{formatCurrency(item.tradePrice || item.unitPrice)}</td>
                      <td className="py-2.5 px-2 text-right text-muted-foreground">{formatCurrency(item.mrp || item.unitPrice)}</td>
                      <td className="py-2.5 px-2 text-right font-bold text-foreground">{formatCurrency(item.totalAmount)}</td>
                    </>
                  )}
                </tr>
              ))}
        ...
        {activeTab === "INVOICE" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-border/80 text-xs">
              ...
              {invoice.paymentAllocations && invoice.paymentAllocations.length > 0 ? (
                <div className="space-y-1.5">
                  {invoice.paymentAllocations.map((p, i) => (
                    <div
                      key={i}
                      className="p-2.5 rounded-xl bg-muted/20 border border-border/60 flex items-center justify-between text-xs"
                    >
                      <div>
            ...
            <div className="space-y-2 font-mono sm:pl-8">
              ...
              <div className="pt-2 border-t-2 border-foreground flex justify-between font-bold text-sm text-foreground">
        ...
        <div className="pt-16 space-y-12 text-xs">
          {/* Signatures */}
          <div className="grid grid-cols-3 gap-6 text-center text-muted-foreground">
            <div className="border-t border-border/80 pt-2">
              <p className="font-semibold text-foreground">Warehouse / Dispensing Officer</p>
              <p className="text-[10px]">Apex Pharma Dist.</p>
            </div>

            <div className="border-t border-border/80 pt-2">
              <p className="font-semibold text-foreground">Sales Representative</p>
              <p className="text-[10px]">Dispatch In-charge</p>
            </div>

            <div className="border-t border-border/80 pt-2">
              <p className="font-semibold text-foreground">Customer Pharmacy Receiver</p>
              <p className="text-[10px]">Signature & Official Seal</p>
            </div>
          </div>

          {/* DGDA Compliance Notice */}
          <div className="text-[10px] text-muted-foreground border-t border-border/40 pt-3 text-center">
